...
token = input('Which coin flow would you like?')
pair = token + '/USD'
plot_color = 'cyan'
...
while i > 0 :
    ...
    if len(PRICE_TIME) > 0:
        ...
        if delta_plotted_time.seconds/3600 > x_hrs:
            PRICE_TIME = PRICE_TIME[1:]
            roll_switch = True
            roll_count = roll_count + 1
            plt.xlim(PRICE_TIME[0],PRICE_TIME[-1])
            count = count % 3600
            coin_matrix = np.concatenate(coin_matrix[0,(count+1):],coin_matrix[0,:count])

    PRICE_TIME.append(time_start)
    count = count + 1

    # Fetch price
    current_price = ctools.fetch_price_data_API(symbol=pair, start = time_start)
